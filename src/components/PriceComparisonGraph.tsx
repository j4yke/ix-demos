...
const SVG_PAD_X = 110 // horizontal breathing room on each side
...
/** Straight line path between points */
...
export default function PriceComparisonGraph() {
  ...
  // Pan spring — shifts SVG so active stage column aligns with centerX
  const panX = useSpring(-stageX(0) + centerX, PAN_SPRING)

  // Label Y springs — follow the curve at the active stage
  const othersY   = useSpring(graph.othersYs[0],   LABEL_SPRING)
  const autosendY = useSpring(graph.autosendYs[0], LABEL_SPRING)

  useEffect(() => {
    panX.set(-stageX(stage) + centerX)
    othersY.set(graph.othersYs[stage])
    autosendY.set(graph.autosendYs[stage])
  }, [stage, centerX, graph.othersYs, graph.autosendYs, panX, othersY, autosendY])
  ...
  // Create an array of stage indices from -1 to STAGES.length for grid lines
  const gridLineIndices = [-1, ...STAGES.map((_, i) => i), STAGES.length]

  return (
    <div className="price-graph-root">

      {/* ── Graph content area (clips the panning SVG) ──────────────────── */}
      <div ref={containerRef} className="price-graph-content">
        ...
        {/* ── Price labels — pinned to centerX, Y animates with spring ─── */}

        {/* Other ESPs label — sits directly ON its data point */}
        <motion.div
          ...
          style={{ left: centerX, top: othersY, x: '-50%', y: '-50%' }}
        >
        ...
        {/* AutoSend label — sits directly ON its data point */}
        <motion.div
          ...
          style={{ left: centerX, top: autosendY, x: '-50%', y: '-50%' }}
        >
      ...
      <div className="h-[56px] shrink-0 flex items-center bg-[#e7e5e4] w-full border-t border-[#e7e5e4]">
        ...
        <button
          className="w-[84px] h-full flex items-center justify-center bg-[#fafaf9] hover:bg-[#f5f5f4] transition-colors shrink-0 border-r border-[#e7e5e4] cursor-pointer"
        ...
        <button
          className="w-[84px] h-full flex items-center justify-center bg-[#fafaf9] hover:bg-[#f5f5f4] transition-colors shrink-0 border-l border-[#e7e5e4] cursor-pointer"
          onClick={() => goTo(stage + 1)}
